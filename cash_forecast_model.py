...
class CashForecastingPipeline:
    ...
    def iterative_forecast(self, country_code: str, model_name: str, train_file: str,
                           template_file: str) -> pd.DataFrame:
        """Implement proper one-step-ahead forecasting"""
        # Load initial data and ensure datetime type for dates
        ...
        # Define key columns for identification
        key_columns = ['ProductId', 'BranchId', 'CountryCode', 'Currency']
        key_columns_with_date = key_columns + ['EffectiveDate']
        ...
        for forecast_date in forecast_dates:
            ...
            # Drop any potential duplicates in both current and inference data
            current_data = current_data.drop_duplicates(subset=key_columns_with_date)
            inference_data = inference_data.drop_duplicates(subset=key_columns)

            # Combine with current data
            combined_data = pd.concat([current_data, inference_data], ignore_index=True)
            combined_data = combined_data.drop_duplicates(subset=key_columns_with_date, keep='last')

            # Ensure all required columns are present
            required_columns = key_columns + ['EffectiveDate', 'Demand']
            for col in required_columns:
                if col not in combined_data.columns:
                    raise ValueError(f"Required column '{col}' missing from combined data.")
            ...
            if forecast is not None:
                ...
                # Merge forecasted demand back to current data
                forecast_with_date = forecast.assign(EffectiveDate=forecast_date)
                forecast_with_data = pd.merge(
                    inference_data,
                    forecast_with_date[key_columns + ['EffectiveDate', 'ForecastDemand']],
                    on=key_columns + ['EffectiveDate'],
                    how='left'
                )
                ...
                # Update current data with the new forecasted demand
                current_data = pd.concat([current_data, forecast_with_data], ignore_index=True)
                current_data = current_data.drop_duplicates(subset=key_columns_with_date, keep='last')
            else:
                self.logger.warning(f"No forecast generated for date {forecast_date}")

        if not all_forecasts:
            raise ValueError("No forecasts were generated during the iterative process")

        # Compile all forecasts
        final_forecasts = pd.concat(all_forecasts, ignore_index=True)

        # Ensure final forecasts have all required columns
        expected_columns = key_columns + ['EffectiveDate', 'ForecastDemand']
        missing_columns = [col for col in expected_columns if col not in final_forecasts.columns]
        if missing_columns:
            raise ValueError(f"Final forecasts missing columns: {missing_columns}")
