...
class CashForecastingInference:
    ...
    def load_scaling_parameters(self, country_code: str, model_timestamp: str) -> None:
        """Load and validate scaling parameters used during training."""
        try:
            # Construct paths
            base_path = f"./cs_data/cash/{country_code}_{model_timestamp}"
            scaling_file = os.path.join(base_path, "scaling_params.json")
            metadata_file = os.path.join(base_path, "scaling_metadata.json")

            # Validate file existence
            for file_path in [scaling_file, metadata_file]:
            ...
            # Log success
            self.logger.info(f"Successfully loaded scaling parameters for {country_code}")
            self.logger.info(f"Scaling method: {self.scaling_metadata['scaling_method']}")
            self.logger.info(f"Number of scaling groups: {len(self.scaling_params)}")

            # Validate metadata structure
            required_metadata = {'scaling_method', 'scaling_level', 'scaled_column'}
            ...
            raise ValueError(f"Invalid JSON in scaling files: {str(e)}")
        except Exception as e:
            self.logger.error(f"Error loading scaling parameters: {str(e)}")
            raise
    ...
    def prepare_inference_data(self,
        ...
        try:
            ...
            if not all(col in inference_template.columns for col in required_columns):
                raise ValueError(f"Missing required columns in template: {required_columns}")
            ...
            future_dates = [effective_date + pd.Timedelta(days=i)
                            for i in range(1, self.config.forecast_horizon + 1)]

            # Create inference data
            inference_data = pd.DataFrame({
                'ProductId': np.tile(inference_template['ProductId'].values,
                                     self.config.forecast_horizon),
                'BranchId': np.tile(inference_template['BranchId'].values,
                                    self.config.forecast_horizon),
                'Currency': np.tile(inference_template['Currency'].values,
                                    self.config.forecast_horizon),
            ...
            template_combinations = set(zip(inference_template['Currency'],
                                            inference_template['BranchId']))
            ...
            self.logger.error(f"Error preparing inference data: {str(e)}")
            raise
    ...
    def _generate_statistical_report(self, result_df: pd.DataFrame, country_code: str) -> None:
        ...
        try:
            ...
            for (currency, branch), params in self.scaling_params.items():
                mask = ((result_df['Currency'] == currency) &
                        (result_df['BranchId'] == branch))
                ...
                for quantile in self.config.quantiles:
                    ...
                    zscore = np.abs((values - params['mean']) /
    ...
    def _rollback_scaling(self) -> None:
        ...
        if self.scaling_state and not self.scaling_state.successful:
            ...
            try:
                ...
                self.logger.error(f"Failed to rollback scaling: {str(e)}")
...
def _save_forecasts(self, country_code: str, forecasts_df: pd.DataFrame) -> None:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"Error converting date columns: {str(e)}")
        ...
        for quantile in self.config.quantiles:
            try:
                ...
                raise ValueError(f"Error converting {quantile} to numeric: {str(e)}")
        ...
        try:
            ...
            self.logger.error(f"Error in pivot and save operations: {str(e)}")
            raise

    except Exception as e:
        self.logger.error(f"Error in forecast saving process: {str(e)}")
        # Try to save error state if possible
        try:
            error_file = os.path.join(output_path, f"error_state_{self.timestamp}.json")
            error_state = {
                'timestamp': self.timestamp,
                'error': str(e),
                'error_type': type(e).__name__,
                'forecast_shape': forecasts_df.shape if isinstance(forecasts_df, pd.DataFrame) else None
            }
            with open(error_file, 'w') as f:
                json.dump(error_state, f, indent=2)
            self.logger.info(f"Saved error state to: {error_file}")
        except Exception as nested_e:
            self.logger.error(f"Failed to save error state: {str(nested_e)}")
        raise
    ...
    try:
        ...
        for country_code in args.countries:
            ...
            try:
                ...
                inference_pipeline.load_scaling_parameters(
                    country_code,
